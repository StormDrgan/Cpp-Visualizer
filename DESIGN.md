...
│  ┌─────────────┐  ┌──────────────────┐  ┌──────────────────────┐ │
│  │ Monaco      │  │ 中央可视化画布    │  │ 右侧面板              │ │
│  │ Editor      │  │ (Canvas / SVG)   │  │ - 变量列表           │ │
│  │             │  │                  │  │ - 数据结构检视       │ │
│  │ 高亮当前行  │  │ 链表/树/数组/图  │  │ - 历史步骤列表       │ │
│  │ 断点标记    │  │ 的图形化展示     │  │ - 用户标注配置       │ │
│  └─────────────┘  └──────────────────┘  └──────────────────────┘ │
│                                                                    │
│  ┌────────────────────────────────────────────────────────────┐   │
...
│  executionState = {                                                │
│    currentLine: number,                                            │
│    history: StateSnapshot[],  // 状态快照栈（历史）                 │
│    future: StateSnapshot[],   // 前进栈（后退后又往前的部分）       │
...
│                                                                    │
│  ┌───────────────┐  ┌────────────────┐  ┌─────────────────────┐  │
│  │ Session       │  │ GDB Controller │  │ Code Compiler       │  │
│  │ Manager       │  │                │  │                     │  │
│  │               │  │ - 起停 GDB 进程 │  │ - 接收用户代码      │  │
│  │ - 用户会话管理 │  │ - step/next    │  │ - 写入临时文件      │  │
│  │ - 超时清理    │  │ - break/watch   │  │ - clang++/g++ 编译  │  │
│  │ - 资源隔离    │  │ - info locals   │  │ - 返回编译错误      │  │
│  │               │  │ - 执行表达式    │  │                     │  │
│  └───────────────┘  └────────────────┘  └─────────────────────┘  │
│                                                                    │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │ 内存遍历器（Memory Walker）                                    │  │
│  │                                                               │  │
│  │ - 根据用户标注的数据结构类型，自动遍历 heap 对象              │  │
│  │ - 链表：从 head 变量出发，沿 next 逐节点读取                  │  │
│  │ - 二叉树：从 root 出发，沿 left/right 递归遍历               │  │
│  │ - 检测环（快慢指针法）→ 标记环路                              │  │
│  │ - 返回结构化数据（节点地址 + 字段值 + 邻接关系）              │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                                                    │
│  后端存储（无持久化数据库，全在内存 / 临时文件）：                  │
│  - 临时源码文件（/tmp/cpp-viz/<session-id>/）                     │
│  - 编译产物                                                       │
│  - GDB 进程                                                       │
└──────────────────────────────────────────────────────────────────┘
```
  ...
  "locals": [
    {
      ...
      "display_value": "ListNode(val=3, next=→)",
      "is_pointer": true,
      "deref_type": "ListNode"
    },
    {
...
```
┌──────────────────────────────────────────────────────────────────────┐
│ Header: 标题 + 主题切换 + 会话状态指示器（● 运行中 / ● 暂停 / ● 结束）│
├───────────────────┬───────────────────────┬──────────────────────────┤
│                   │                       │                          │
│   代码编辑区       │    可视化主画布        │     信息面板              │
│  (Monaco Editor)  │   (Canvas 全宽)       │                          │
│                   │                       │  ┌────────────────────┐  │
│  ┌──────────────┐ │                       │  │ 📋 局部变量        │  │
│  │              │ │   ┌───────────────┐   │  │ name  type  value  │  │
│  │  1  ListNode │ │   │  [3]→[4]→[5]  │   │  │ slow  *Node 0x..  │  │
│  │  2  *merge(..│ │   │   ↑      ↑    │   │  │ fast  *Node 0x..  │  │
│  │  3    slow = │ │   │  slow   fast  │   │  └────────────────────┘  │
│  │  4    while..│ │   │               │   │                          │
│  │> 5    fast = │◀──┼── 执行到第5行   │   │  ┌────────────────────┐  │
│  │  6    ...    │ │   │               │   │  │ 🔍 监视表达式      │  │
...
├───────────────────┴───────────────────────┴──────────────────────────┤
│ 控制栏                                                               │
│ [⏮ 回到起点] [◀ 后退一步] [▶ 前进一步] [⏩ 运行到断点] [⏭ Step Over] │
│ [↺ 重置] [速度: ───●───] 当前行: 15  函数: mergeTwoLists            │
└──────────────────────────────────────────────────────────────────────┘
```
...
| 任务 | 预估 |
|---|---|
| Memory Walker - 二叉树 | 递归遍历 left/right |
| 树形布局 | dagre / Reingold-Tilford |
| Canvas 树渲染 | 节点 + 边 + 高亮路径 |
| 树旋转动画 | AVL 旋转时的过渡动画 |
