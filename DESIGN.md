...
### 4.4 用户标注系统（v0.8+ 降级为兜底）

v0.8 起，`auto_discover` 自动处理常见场景。手动标注仅在以下情况需要：
- 字段命名不常规（如 `succ` 而非 `next`）
- 堆数组需要指定长度变量（`new int[n]` 的 `n`）
- 复杂嵌套结构

用户可以在代码中添加特殊注释来告诉系统"这段代码里有什么数据结构需要可视化"。
...
| # | 任务 | 说明 |
|---|------|------|
| **A1** | **排序动画增强** | watched 指针（i, j, minIdx）直接推断比较/交换目标，不依赖 diff 交叉匹配。单步内区分「比较」和「交换」两个阶段，动画更准确 |
| **A2** | **栈/队列操作动画** | push/pop/enqueue/dequeue 节点级动画 — 新元素从上方滑入，弹出元素缩小淡出，队列出队从队首滑出 |
| **A3** | **堆操作动画** | sift-up（上浮）/ sift-down（下沉）节点沿完全二叉树路径交换动画，展示堆调整的完整过程 |
| **A4** | **图遍历动画** | BFS 队列扩张 + 节点染色（层级推进，渐变色）；DFS 递归栈 + 回溯染色（路径高亮，回溯时暗化） |
| **A5** | **递归树可视化** | 递归调用栈渲染为树形结构，每个递归层一个节点，展示分治/回溯的调用树，fibonacci/mergeSort 等经典算法可视化 |

**关键技术实现（A1-A5）：**
- 排序动画：在 `compute_diff` 阶段根据 watched 指针位置直接生成 `element_compared`/`element_swapped`，不靠 diff 交叉匹配
- 栈/队列动画：利用现有 `walk_array`/`walk_linked_list` walker，新增 `node_pushed`/`node_popped` diff action
- 堆动画：数组 walk → Canvas 层按 (2i+1, 2i+2) 计算坐标，swap 动画沿路径逐级闪烁
- 图遍历：新增 `node_colored` diff action → Canvas 层渐变动画（白→蓝 BFS / 白→紫 DFS）
- 递归树：新增 `walk_recursion` walker → 解析调用栈帧 → 生成树形节点 + 父子边

#### 二、体验增强（P1）

| # | 任务 | 说明 |
|---|------|------|
| **B1** | **历史步骤列表（时间轴）** | 左侧/底部步骤时间轴，显示每步行号 + 操作摘要（如「第 8 行：slow = slow->next」），点击任意步骤跳到对应快照。利用现有 history 栈，低成本接入 |
| **B2** | **自动识别增强** | 识别更多结构 — 双向链表（检测 prev 字段）、循环链表（next 回到 head）、AVL 树（检测 balance_factor 字段），减少手动标注场景 |

**关键技术实现（B1-B2）：**
- B1：前端 `HistoryPanel` 组件 → 每次收到 snapshot 追加步骤记录 → 点击步骤 N → `back(steps = cur - N)` 跳转
- B2：`auto_discover` 增强 — 检测 `prev` 字段 → 标记为 doubly_linked_list；检测 `next` 指向 root → 标记为 circular；检测 `height`/`bf` 字段 → 标记为 AVL
...
> **文档版本**：v0.8
> **最后更新**：2026-06-21
> **状态**：v0.8 已完成（auto_discover 自动化 + 可视化目标勾选 + 即时过滤，标注降级为兜底）；v0.9 算法动画 + 体验升级、v0.10 Level 2 数据结构已规划
